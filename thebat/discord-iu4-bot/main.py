...
bot = commands.Bot(command_prefix=BOT_COMMAND_PREFIX, status=discord.Status.online, activity=None)
...
@commands.has_role(PERMITTED_COMMANDS_ROLE)
...
def main():
    ...
    if not os.path.isfile(DISCORD_TOKEN_FILEPATH):
        sys.exit(f"Discord token file not found: {DISCORD_TOKEN_FILEPATH}")
    ...
    token = discord_get_token(DISCORD_TOKEN_FILEPATH)
    bot.run(token)
